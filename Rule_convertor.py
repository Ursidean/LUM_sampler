...
act_names = ["Arable land", "Permanent crops",
             "Pastures", "Agricultural areas", "Residential",
             "Industry & commerce", "Recreation areas",
             "Forest"]
...
# Specify the accessibility distance decay and weight values.
...
FKS = fks(omap_path, amap_path, smap_path, fuzzy_trans_coefficients)
# Store the calculated area-weighted clumpiness error.
AWCE = area_weighted_clu_error(amap, smap, mask, luc, pas, act, luc_count)
# Log the tested parameters.
tested_pars = [0] * 376
# Use a counter to track the input parameter.
it_par = 0
# Read the Inertia/self-influence parameters for
# the specific solution into the rule dictionary.
for i in range(0, act):
    key = ("from " + act_names[i] + " to "
           + act_names[i])
    # Specify the inertia parameter.
    tested_pars[it_par] = rules[key][0]
    it_par = it_par + 1
    # Specify the 'a' parameter.
    tested_pars[it_par] = rules[key][1]
    it_par = it_par + 1
    # Specify the 'b' parameter.
    tested_pars[it_par] = rules[key][2]
    it_par = it_par + 1
# Read the Conversion/cross-influence parameters for
# the specific solution into the rule dictionary.
for i in range(0, act):
    for j in range(0, luc):
        ...
        if fu_name == lu_name:
            ...
            key = ("from " + lu_name + " to "
                   + fu_name)
            # Specify the conversion parameter.
            tested_pars[it_par] = rules[key][0]
            it_par = it_par + 1
            # Specify the 'a' parameter.
            tested_pars[it_par] = rules[key][1]
            ...
            tested_pars[it_par] = rules[key][2]
            it_par = it_par + 1
# Read the accessibility parameters into the
# required lists.
for i in range(0, act):
    tested_pars[it_par] = acc_dd[i]
    it_par = it_par + 1
for i in range(0, act):
    tested_pars[it_par] = acc_w[i]
    it_par = it_par + 1

# Generate a header list.
header = ["0"] * 376
for i in range(0, 376):
    header[i] = "Par " + str(i + 1)
